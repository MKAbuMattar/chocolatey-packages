...
"@
  ...
  It 'preserves CRLF and LF nuspec line endings' {
    ...
    (Get-FileText (Join-Path $crlfDir 'crlf.nuspec')) | Should -Be "<package>`r`n  <description><![CDATA[`r`nbody`r`n]]></description>`r`n</package>`r`n"
    (Get-FileText (Join-Path $lfDir 'lf.nuspec')) | Should -Be "<package>`n  <description><![CDATA[`nbody`n]]></description>`n</package>`n"
  }
